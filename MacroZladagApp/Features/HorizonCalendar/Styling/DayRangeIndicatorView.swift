...
final class DayRangeIndicatorView: UIView {
    ...
    override func draw(_ rect: CGRect) {
        ...
        for dayRowFrame in dayRowFrames {
            let tempDayRowFrame = CGRect(
                x: dayRowFrame.minX,
                y: dayRowFrame.minY + 4,
                width: dayRowFrame.width,
                height: 28
            )
            let roundedRectanglePath = UIBezierPath(roundedRect: tempDayRowFrame, cornerRadius: 20)
