...
//        navigationController?.navigationBar.backgroundColor = .customOrange
...
//            header.bottomAnchor.constraint(equalTo: view.bottomAnchor),
    ...
    private func createSwitchOnboardPageStack() -> UIStackView {
        let label = createDefaultLabel("Sudah punya akun?")
        label.setContentCompressionResistancePriority(.required, for: .horizontal)
        label.setContentHuggingPriority(.required, for: .horizontal)
        
        
        switchOnboardPageButton = createDefaultButton("")
        let attributedText = NSMutableAttributedString(string: "Masuk")
